...
contract HoprStakeSeason8Test is Test, ERC1820RegistryFixtureTest, PermittableTokenFixtureTest {
    ...
    function setUp() public virtual override {
        ...
        hoprStakeSeason8 = new HoprStakeSeason8(OWNER, NFT_ADDRESS, LOCK_TOKEN_ADDRESS, REWARD_TOKEN_ADDRESS);
    ...
    function test_RightParameters() public {
        assertEq(OWNER, hoprStakeSeason8.owner());
        assertEq(NFT_ADDRESS, address(hoprStakeSeason8.NFT_CONTRACT()));
        assertEq(LOCK_TOKEN_ADDRESS, hoprStakeSeason8.LOCK_TOKEN());
        assertEq(REWARD_TOKEN_ADDRESS, hoprStakeSeason8.REWARD_TOKEN());
    }
    ...
    function testRevert_WhenNotOwnerCannotStakeForOthers(address caller) public {
        ...
        hoprStakeSeason8.batchStakeFor(stakeForAccounts, stakeForStakes);
    }
    ...
    function testRevert_WhenNotOwnerCannotStakeForOthers() public {
        _helperSetAccountsAndStakes(3, 3);
        vm.warp(hoprStakeSeason8.PROGRAM_END() + 1);
    ...
    function testRevert_WhenNotEnoughApprovedTokensBatchStakeFor(uint256 accLength) public {
        ...
        vm.mockCall(
            LOCK_TOKEN_ADDRESS,
            abi.encodeWithSignature("transferFrom(address,address,uint256)", OWNER, address(hoprStakeSeason8), amount),
            abi.encode(false)
        );
        vm.expectRevert();
        hoprStakeSeason8.batchStakeFor(stakeForAccounts, stakeForStakes);
    ...
    function test_batchStakeFor(uint256 accLength) public {
        ...
        vm.store(
            ...
            bytes32(
                stdstore.target(address(LOCK_TOKEN_ADDRESS))
                    ...
                    .with_key(OWNER)
                    .with_key(address(hoprStakeSeason8))
                    .find()
            ),
            bytes32(abi.encode(amount))
        );
        (bool successReadAllowance, bytes memory returndatallowance) = LOCK_TOKEN_ADDRESS.staticcall(
            abi.encodeWithSignature("allowance(address,address)", OWNER, address(hoprStakeSeason8))
        );
        ...
        for (uint256 index = 0; index < accLength; index++) {
            vm.expectEmit(true, true, false, false, address(hoprStakeSeason8));
        ...
        for (uint256 j = 0; j < accLength; j++) {
            (uint256 actualLockedAfterBatchStake,,,) = hoprStakeSeason8.accounts(stakeForAccounts[j]);
            assertEq(actualLockedAfterBatchStake, stakeForStakes[j]);
        }
        // caller doesn't have stake
        (uint256 actualLockedOfCaller,,,) = hoprStakeSeason8.accounts(OWNER);
        assertEq(actualLockedOfCaller, 0);
        // check the total stake in the contract equals to the provided value
        assertEq(hoprStakeSeason8.totalLocked(), amount);
        ...
        (bool successReadBalanceOfStake, bytes memory returndatBalanceOfStake) =
            LOCK_TOKEN_ADDRESS.staticcall(abi.encodeWithSignature("balanceOf(address)", address(hoprStakeSeason8)));
    ...
    function test_batchBlockNfts(uint256[] calldata blockedIndexes) public {
        ...
        for (uint256 index = 0; index < blockedIndexes.length; index++) {
            vm.expectEmit(true, false, false, false, address(hoprStakeSeason8));
            emit NftBlocked(blockedIndexes[index]);
        }
        hoprStakeSeason8.ownerBatchBlockNftType(blockedIndexes);
        vm.clearMockedCalls();
    }
    ...
    function testRevert_batchBlockNfts(uint256[] calldata blockedIndexes) public {
        ...
        for (uint256 k = 0; k < blockedIndexes.length; k++) {
            if (hoprStakeSeason8.isBlockedNft(blockedIndexes[k])) {
    ...
    function test_batchUnblockNfts(uint256[] calldata blockedIndexes) public {
        ...
        for (uint256 k = 0; k < blockedIndexes.length; k++) {
            if (!hoprStakeSeason8.isBlockedNft(blockedIndexes[k])) {
                hoprStakeSeason8.ownerBlockNftType(blockedIndexes[k]);
            }
        }
        ...
        hoprStakeSeason8.ownerBatchUnblockNftType(blockedIndexes);
        vm.stopPrank();
        vm.clearMockedCalls();
    }
